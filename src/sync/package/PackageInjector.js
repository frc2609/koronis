...
export const inject = (inYear) => {
  ...
  let fieldStateDefinition = {
    ...
    init: ( //Mirror both zones and drawn elements across the y axis in the middle
      'let newZones = [];' +
      'let len = fS.zones.length;' +
      'for(let i = 0;i < len;i++) {' +
      '  let currZone = Object.assign({},fS.zones[i]);' +
      ...
      'len = dE.length;' +
      'for(let i = 0;i < len;i++) {' +
      '  let currDrawnElement = Object.assign({}, dE[i]);' +
  ...
  if(availableYears !== null || typeof availableYears !== 'undefined') {
